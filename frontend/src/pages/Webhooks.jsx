    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-dark-400 text-sm mt-1">Configure Slack, Discord, or custom webhook notifications</p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Webhooks</h1>
            <p className="text-dark-400 text-sm">Configure Slack, Discord, or custom webhook notifications</p>
          </div>
        </div>
        <button
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-5 animate-fade-in">
          ...
          <form onSubmit={handleCreate} className="space-y-4" noValidate>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-dark-300 mb-1" htmlFor="wh-name">Name</label>
                <input
                  id="wh-name"
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-sm text-dark-300 mb-1" htmlFor="wh-type">Type</label>
                <select
                  id="wh-type"
                  value={form.type}
            ...
            <div>
              <label className="block text-sm text-dark-300 mb-1" htmlFor="wh-url">Webhook URL</label>
              <input
                id="wh-url"
                type="url"
                value={form.url}
            ...
            <div>
              <label className="block text-sm text-dark-300 mb-2">Events to notify</label>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-dark-400 truncate max-w-xs">{webhook.url}</p>
                  </div>
                </div>
              ...
              {webhook.events?.length > 0 && (
                <div className="flex gap-1.5 mt-2 ml-9">
                  {webhook.events.map(event => (
                    <span key={event} className="text-xs bg-dark-700 text-dark-300 px-2 py-0.5 rounded">
                      {event.replace('_', ' ')}
                    </span>
                  ))}
                </div>
