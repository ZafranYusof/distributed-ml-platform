...
export default function History() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return 'text-purple-300/50';
    }
  };
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart>
                  ...
                  <XAxis dataKey="epoch" stroke="#A78BFA" fontSize={12} type="number" domain={[1, 'dataMax']} allowDuplicatedCategory={false} />
                  <YAxis stroke="#A78BFA" fontSize={12} />
                  <Tooltip contentStyle={{ backgroundColor: '#1E1045', border: '1px solid #2d1b69', borderRadius: '8px' }} labelStyle={{ color: '#e2e8f0' }} />
                  <Legend />
              ...
              <table className="w-full text-sm" role="table" aria-label="Configuration comparison">
                <thead>
                  <tr className="border-b border-purple-500/20">
                    <th className="text-left py-2 px-3 text-purple-300/50" scope="col">Property</th>
                ...
                <tbody>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Architecture</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-dark-200">{s.config?.type}</td>)}</tr>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Learning Rate</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-dark-200">{s.config?.learningRate}</td>)}</tr>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Epochs</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-dark-200">{s.config?.epochs}</td>)}</tr>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Batch Size</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-dark-200">{s.config?.batchSize}</td>)}</tr>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Final Loss</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-purple-400">{s.finalMetrics?.loss?.toFixed(6) || 'N/A'}</td>)}</tr>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Final Accuracy</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-green-400">{s.finalMetrics?.accuracy?.toFixed(4) || 'N/A'}</td>)}</tr>
                  <tr className="border-b border-dark-800"><td className="py-2 px-3 text-purple-200/70">Workers</td>{compareData.map(s => <td key={s._id} className="py-2 px-3 text-dark-200">{s.config?.numWorkers}</td>)}</tr>
                </tbody>
              </table>
            </div>
          </div>
        ...
        <>
          {sessions.length === 0 ? (
            ...
            <div className="space-y-3">
              <p className="text-sm text-purple-300/50">Select 2+ sessions to compare them side by side</p>
              {sessions.map(session => (
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 rounded border-purple-500/30 bg-dark-800/40 text-primary-500 focus:ring-primary-500"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-1 text-xs text-purple-300/50">
                      <span>{session.config?.type || 'unknown'}</span>
                      <span>{session.datasetName || 'Unknown dataset'}</span>
                      <span>{new Date(session.startedAt).toLocaleString()}</span>
                      {session.finalMetrics?.loss && (
                        <span className="text-purple-400">Loss: {session.finalMetrics.loss.toFixed(6)}</span>
                      )}
