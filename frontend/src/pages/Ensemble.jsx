...
export default function Ensemble() {
  ...
  if (!user) return <div className="text-purple-300/50 text-center py-20">Sign in to use Ensemble Methods</div>;
        ...
        <div className="lg:col-span-2 bg-dark-800/40 border border-purple-500/20 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <label className="text-purple-300/50 text-sm">Top N:</label>
              ...
              <button onClick={autoSelect} className="px-3 py-1 bg-purple-500/10 text-purple-300 border border-primary-500/20 rounded-lg text-sm hover:bg-primary-500/20">
                Auto-Select
              </button>
            </div>
          </div>
          {loading ? (
            <p className="text-purple-300/50">Loading experiments...</p>
          ) : (
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {experiments.map(exp => (
                <div key={exp._id} onClick={() => toggleModel(exp._id)}
                  className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${selectedModels.includes(exp._id) ? 'bg-primary-500/10 border border-primary-500/30' : 'bg-dark-800/50 backdrop-blur-md border border-purple-500/20 hover:border-dark-500'}`}>
                  <div className="flex items-center gap-3">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={m.id} onClick={() => setMethod(m.id)}
                className={`p-3 rounded-lg cursor-pointer transition-colors ${method === m.id ? 'bg-primary-500/10 border border-primary-500/30' : 'bg-dark-800/50 backdrop-blur-md border border-purple-500/20'}`}>
                <p className="text-dark-200 text-sm font-medium">{m.label}</p>
                <p className="text-purple-300/40 text-xs mt-1">{m.desc}</p>
              </div>
            ))}
          </div>
          <button onClick={runEnsemble} disabled={selectedModels.length < 2}
            className="w-full mt-4 px-4 py-2 bg-gradient-btn text-white rounded-lg hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed">
            Run Ensemble
          </button>
        </div>
      </div>
          ...
          <div className="bg-dark-800/40 border border-purple-500/20 rounded-xl p-6">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={results.comparison}>
                ...
                <XAxis dataKey="name" stroke="#6b5b95" tick={{ fontSize: 11 }} />
                ...
                <Bar dataKey="accuracy" radius={[4, 4, 0, 0]}>
                  {results.comparison.map((entry, i) => (
                    <Cell key={i} fill={i === results.comparison.length - 1 ? '#06b6d4' : '#6b5b95'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="bg-dark-800/40 border border-purple-500/20 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div className="bg-dark-900 rounded-lg p-4">
                <p className="text-purple-300/50 text-xs">Ensemble Accuracy ({results.method})</p>
                <p className="text-3xl font-bold text-purple-400">{(results.ensembleAcc * 100).toFixed(1)}%</p>
              </div>
              <div className="bg-dark-900 rounded-lg p-4">
                <p className="text-purple-300/50 text-xs">Best Individual</p>
                <p className="text-2xl font-bold text-dark-200">{(Math.max(...results.individualAccs) * 100).toFixed(1)}%</p>
              </div>
              <div className="bg-dark-900 rounded-lg p-4">
                <p className="text-purple-300/50 text-xs">Improvement</p>
