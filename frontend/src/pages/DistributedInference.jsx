  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-dark-400 mt-1">Split batch predictions across multiple Web Workers for parallel inference</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-4">
          ...
          <input type="number" value={batchSize} onChange={(e) => setBatchSize(Number(e.target.value))} min={10} max={10000} className="w-full bg-dark-900 border border-dark-600 text-dark-200 rounded-lg px-3 py-2 text-sm" />
        </div>
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-4">
          <label className="block text-sm text-dark-400 mb-2">Number of Workers</label>
          <input type="number" value={numWorkers} onChange={(e) => setNumWorkers(Number(e.target.value))} min={1} max={16} className="w-full bg-dark-900 border border-dark-600 text-dark-200 rounded-lg px-3 py-2 text-sm" />
        </div>
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-4">
          <label className="block text-sm text-dark-400 mb-2">Input Features</label>
          <input type="number" value={modelConfig.inputSize} onChange={(e) => setModelConfig(prev => ({ ...prev, inputSize: Number(e.target.value) }))} min={1} max={100} className="w-full bg-dark-900 border border-dark-600 text-dark-200 rounded-lg px-3 py-2 text-sm" />
        </div>
      </div>
            ...
            {workerStats.map(w => (
              <div key={w.id} className="bg-dark-900 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-dark-300">Worker {w.id}</span>
                ...
                <div className="w-full bg-dark-700 rounded-full h-2 mb-2">
                  <div className="bg-primary-500 h-2 rounded-full transition-all" style={{ width: `${w.utilization}%` }}></div>
                </div>
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-dark-900 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-primary-400">{results.throughput.toFixed(0)}</p>
              <p className="text-xs text-dark-400 mt-1">predictions/sec</p>
            </div>
            <div className="bg-dark-900 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{results.totalTime.toFixed(1)}ms</p>
              <p className="text-xs text-dark-400 mt-1">total time</p>
            </div>
            <div className="bg-dark-900 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-yellow-400">{results.avgLatencyPerBatch.toFixed(1)}ms</p>
              <p className="text-xs text-dark-400 mt-1">avg latency/worker</p>
            </div>
            <div className="bg-dark-900 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-white">{results.totalSamples}</p>
              <p className="text-xs text-dark-400 mt-1">total predictions</p>
            </div>
          </div>
          {results.predictions.length > 0 && (
            <div className="mt-4">
              <p className="text-sm text-dark-400 mb-2">Sample predictions (first 5):</p>
              <div className="bg-dark-900 rounded-lg p-3 text-xs text-dark-300 font-mono overflow-auto max-h-32">
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-lg p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {history.map((h, i) => (
                  <tr key={i} className="border-b border-dark-700/50 text-dark-300">
                    ...
                    <td className="py-2 px-3 text-primary-400">{h.throughput.toFixed(0)} pred/s</td>
