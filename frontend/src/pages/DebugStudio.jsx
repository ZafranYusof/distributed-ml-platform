  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-dark-400 mt-1">Step through predictions, inspect activations, and analyze failures</p>
      </div>
      ...
      <div className="bg-dark-800 rounded-xl p-6 border border-dark-700">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="text-xs text-dark-400">Activation</label>
        ...
        <button onClick={loadModel} className="px-6 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600">
        ...
        <>
          ...
          <div className="bg-dark-800 rounded-xl p-6 border border-dark-700">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-dark-300 uppercase">Prediction Inspector</h3>
              <div className="flex items-center gap-3">
                <button onClick={() => stepTo(currentIdx - 1)} disabled={currentIdx === 0}
                  className="px-3 py-1 bg-dark-700 text-white rounded disabled:opacity-50">←</button>
                <span className="text-sm text-dark-400">{currentIdx + 1} / {predictions.length}</span>
            ...
            {current && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-dark-900 rounded-lg p-4">
                  <p className="text-xs text-dark-400 mb-2">Input Features</p>
                ...
                <div className="bg-dark-900 rounded-lg p-4">
                  <p className="text-xs text-dark-400 mb-2">Prediction</p>
                  <div className="space-y-2">
                    {current.probs.map((prob, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <span className="text-xs text-dark-300 w-20">{classes[i]}</span>
                        <div className="flex-1 h-4 bg-dark-700 rounded overflow-hidden">
                          <div className={`h-full ${i === current.predicted ? 'bg-primary-500' : 'bg-dark-500'}`}
                ...
                <div className="bg-dark-900 rounded-lg p-4">
                  <p className="text-xs text-dark-400 mb-2">Result</p>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-dark-300 text-sm">True:</span>
                      <span className="text-white text-sm font-medium">{current.trueName}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-dark-300 text-sm">Predicted:</span>
          ...
          {activations.length > 0 && (
            <div className="bg-dark-800 rounded-xl p-6 border border-dark-700">
              <h3 className="text-sm font-semibold text-dark-300 uppercase mb-4">Layer Activations (Heatmap)</h3>
              <div className="space-y-3">
                {activations.map((layer, li) => (
                  <div key={li}>
                    <p className="text-xs text-dark-400 mb-1">Layer {li + 1} ({layer.length} neurons)</p>
                    <div className="flex gap-1 flex-wrap">
                      {layer.map((val, ni) => {
                        ...
                        return (
                          <div key={ni} className="w-8 h-8 rounded flex items-center justify-center text-xs border border-dark-600"
          ...
          {confusionMatrix && (
            <div className="bg-dark-800 rounded-xl p-6 border border-dark-700">
              <h3 className="text-sm font-semibold text-dark-300 uppercase mb-4">Confusion Matrix</h3>
              <div className="overflow-x-auto">
                <table className="mx-auto">
                  <thead>
                    <tr>
                      <th className="p-2"></th>
                      {classes.map(c => <th key={c} className="p-2 text-xs text-dark-400">{c}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {confusionMatrix.map((row, i) => (
                      <tr key={i}>
                        <td className="p-2 text-xs text-dark-400">{classes[i]}</td>
                        {row.map((val, j) => (
                          <td key={j} className="p-2">
                            <div className={`w-12 h-12 flex items-center justify-center rounded text-sm font-bold ${
                              i === j ? 'bg-green-500/30 text-green-400' : val > 0 ? 'bg-red-500/30 text-red-400' : 'bg-dark-700 text-dark-500'
                            }`}>{val}</div>
                          </td>
                        ))}
                      </tr>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Misclassified */}
            <div className="bg-dark-800 rounded-xl p-6 border border-dark-700">
              <h3 className="text-sm font-semibold text-dark-300 uppercase mb-4">Misclassified ({misclassified.length})</h3>
              <div className="max-h-60 overflow-y-auto space-y-2">
                {misclassified.length === 0 ? (
                  <p className="text-dark-400 text-sm">No misclassifications!</p>
                ) : misclassified.map((m, i) => (
                  <div key={i} className="flex items-center justify-between p-2 bg-dark-900 rounded-lg cursor-pointer hover:bg-dark-700"
                    onClick={() => stepTo(m.id)}>
                    <span className="text-xs text-dark-400">#{m.id}</span>
                    <span className="text-xs text-white">{m.trueName}</span>
                    <span className="text-xs text-red-400">→ {classes[m.predicted]}</span>
                    <span className="text-xs text-dark-500">conf: {(m.probs[m.predicted] * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Slice Analysis */}
            <div className="bg-dark-800 rounded-xl p-6 border border-dark-700">
              <h3 className="text-sm font-semibold text-dark-300 uppercase mb-4">Slice Performance</h3>
              <div className="space-y-3">
                {sliceResults.map((s, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="text-xs text-dark-300 w-36 truncate">{s.name}</span>
                    <div className="flex-1 h-4 bg-dark-700 rounded overflow-hidden">
                      <div className="h-full bg-primary-500" style={{ width: `${s.accuracy}%` }}></div>
                    </div>
                    <span className="text-xs text-white w-16 text-right">{s.accuracy}%</span>
                    <span className="text-xs text-dark-500">n={s.count}</span>
                  </div>
                ))}
              </div>
            </div>
