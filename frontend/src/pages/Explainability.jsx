...
export default function Explainability() {
  ...
  if (!user) return <div className="text-purple-300/50 text-center py-20">Sign in to access Explainability Dashboard</div>;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {(tab === 'lime' || tab === 'counterfactual') && (
        <div className="bg-dark-800/40 border border-purple-500/20 rounded-xl p-4">
          <label className="text-purple-300/50 text-sm">Select Experiment:</label>
          <select onChange={e => {
            const exp = experiments.find(x => x._id === e.target.value);
            setSelectedExp(exp);
          }} className="ml-3 bg-dark-900 border border-purple-500/30 rounded-lg px-3 py-1.5 text-dark-200 text-sm">
            ...
            <div className="bg-dark-800/40 border border-purple-500/20 rounded-xl p-6">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={explanations} layout="vertical">
                  ...
                  <XAxis type="number" stroke="#6b5b95" />
                  <YAxis dataKey="feature" type="category" stroke="#6b5b95" width={120} />
        ...
        <div className="space-y-4">
          ...
          {counterfactuals && (
            <div className="bg-dark-800/40 border border-purple-500/20 rounded-xl p-6">
              ...
              <p className="text-purple-300/50 text-sm mb-4">"What minimal change would flip the prediction?"</p>
              <div className="space-y-3">
                {counterfactuals.map((cf, i) => (
                  <div key={i} className="bg-dark-900 rounded-lg p-4 border border-purple-500/20">
                    ...
                    <div className="mt-2 flex items-center gap-3 text-sm">
                      <span className="text-purple-300/50">Original: <span className="text-dark-200">{cf.original}</span></span>
                      <span className="text-purple-300/40">→</span>
                      <span className="text-purple-300/50">Change to: <span className="text-purple-400">{cf.counterfactual}</span></span>
                    </div>
      ...
      {tab === 'boundary' && (
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <span className="text-purple-300/50 text-sm">2D feature space visualization</span>
          </div>
          ...
          {boundaryData && (
            <div className="bg-dark-800/40 border border-purple-500/20 rounded-xl p-6">
              ...
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart>
                  <CartesianGrid strokeDasharray="3 3" stroke="#2d1b69" />
                  <XAxis dataKey="x" name="Feature 0" stroke="#6b5b95" type="number" domain={[0, 10]} />
                  <YAxis dataKey="y" name="Feature 1" stroke="#6b5b95" type="number" domain={[0, 10]} />
                  <Tooltip contentStyle={{ backgroundColor: '#1E1045', border: '1px solid #2d1b69' }} />
              ...
              <div className="flex gap-4 mt-3 text-xs text-purple-300/50">
