...
export default function Schedules() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return 'text-purple-300/50 bg-dark-800/40 border-purple-500/30';
    }
  };
        ...
        <div className="card border-primary-500/20">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-purple-200/70 mb-1">Schedule Name</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-sm text-purple-200/70 mb-1">Cron Expression</label>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-sm text-purple-200/70 mb-2">Quick Presets</label>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <label className="block text-sm text-purple-200/70 mb-1">Model Type</label>
                <select
              ...
              <div>
                <label className="block text-sm text-purple-200/70 mb-1">Learning Rate</label>
                <input
                  type="number"
                  step="0.001"
              ...
              <div>
                <label className="block text-sm text-purple-200/70 mb-1">Epochs</label>
                <input
                  type="number"
                  value={form.epochs}
              ...
              <div>
                <label className="block text-sm text-purple-200/70 mb-1">Batch Size</label>
                <input
                  type="number"
        ...
        <div className="space-y-3">
          {schedules.map(schedule => (
            <div key={schedule._id} className="card">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-purple-300/50 font-mono">{schedule.cronExpression}</p>
                  </div>
                </div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-4 text-xs text-purple-300/50">
                <div>
                  <span className="text-purple-300/40">Last Run:</span>{' '}
                  {schedule.lastRun ? new Date(schedule.lastRun).toLocaleString() : 'Never'}
                </div>
                <div>
                  <span className="text-purple-300/40">Runs:</span>{' '}
                  {schedule.runHistory?.length || 0} total
                </div>
                <div>
                  <span className="text-purple-300/40">Created:</span>{' '}
              ...
              {schedule.runHistory?.length > 0 && (
                <div className="mt-3 border-t border-purple-500/20 pt-3">
                  <p className="text-xs text-purple-300/40 mb-2">Recent Runs:</p>
                  <div className="flex gap-2 overflow-x-auto">
                    {schedule.runHistory.slice(-5).reverse().map((run, i) => (
                      <div key={i} className="flex-shrink-0 bg-dark-800/40 rounded px-3 py-1 border border-purple-500/30">
                        <p className="text-xs text-purple-200/70">{new Date(run.startedAt).toLocaleTimeString()}</p>
