    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-dark-400 text-sm">Connect your repository to version models with commits</p>
        </div>
      </div>
      ...
      {!integration ? (
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
          ...
          <form onSubmit={handleConnect} className="space-y-4 max-w-md">
            ...
            <div>
              <label className="block text-sm text-dark-300 mb-1">Personal Access Token</label>
              <input
                type="password"
                value={token}
                ...
                className="w-full px-3 py-2 bg-dark-700 border border-dark-600 rounded-lg text-white placeholder-dark-400 text-sm focus:outline-none focus:border-primary-500"
              />
              <p className="text-xs text-dark-500 mt-1">Needs repo read access</p>
            </div>
            <button
              type="submit"
              disabled={connecting || !repoUrl || !token}
              className="px-4 py-2 bg-primary-500 text-white rounded-lg text-sm hover:bg-primary-600 disabled:opacity-50 transition-colors"
            >
        ...
        <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-dark-400">{integration.repoUrl}</p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-3">
              {commits.map((commit, i) => (
                <div key={i} className="flex items-start gap-3 p-3 bg-dark-700/50 rounded-lg">
                  <div className="w-8 h-8 rounded-full bg-dark-600 flex items-center justify-center text-xs text-dark-300 flex-shrink-0">
                    {commit.sha.slice(0, 2)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white">{commit.message}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <code className="text-xs text-primary-400 bg-primary-500/10 px-1.5 py-0.5 rounded">{commit.sha}</code>
                      <span className="text-xs text-dark-500">{new Date(commit.date).toLocaleString()}</span>
                    </div>
                  </div>
                  <button className="text-xs text-primary-400 hover:text-primary-300 whitespace-nowrap">
                    Link to model
                  </button>
                </div>
              ))}
          ...
          <div className="bg-dark-800 border border-dark-700 rounded-xl p-5">
            ...
            <p className="text-sm text-dark-400 mb-3">Automatically create a new model version when code is pushed to the repository.</p>
            <div className="flex items-center gap-3">
