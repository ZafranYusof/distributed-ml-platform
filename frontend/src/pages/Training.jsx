    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="card text-center">
          ...
          <p className="text-lg font-bold text-primary-400 mt-1 capitalize">{status}</p>
        </div>
        <div className="card text-center">
          <p className="text-dark-400 text-xs uppercase tracking-wide">Elapsed</p>
          <p className="text-lg font-bold text-dark-100 mt-1">{formatTime(elapsedTime)}</p>
        </div>
        <div className="card text-center">
          <p className="text-dark-400 text-xs uppercase tracking-wide">Workers</p>
          <p className="text-lg font-bold text-dark-100 mt-1">{completedWorkers}/{numWorkers} done</p>
        </div>
        <div className="card text-center">
          <p className="text-dark-400 text-xs uppercase tracking-wide">Speed</p>
          <p className="text-lg font-bold text-green-400 mt-1">{totalSpeed} samples/s</p>
        </div>
        <div className="card text-center">
          <p className="text-dark-400 text-xs uppercase tracking-wide">Architecture</p>
      ...
      {/* Worker Progress */}
      <div className="card">
        <h3 className="text-lg font-semibold text-dark-100 mb-4">👷 Worker Progress</h3>
        <div className="space-y-3">
          {Array.from({ length: numWorkers }, (_, i) => (
            <div key={i} className="space-y-1">
              <div className="flex justify-between text-sm">
                <span className="text-dark-300">Worker {i + 1}</span>
      ...
      {aggregatedLoss.length > 0 && (
        <div className="card">
          <h3 className="text-lg font-semibold text-dark-100 mb-4">📉 Loss Curve</h3>
          <div className="h-72">
      ...
      {metrics.length > 0 && (
        <div className="card">
          <h3 className="text-lg font-semibold text-dark-100 mb-4">📊 Training Metrics</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-dark-700">
                  <th className="text-left py-2 px-3 text-dark-400">Epoch</th>
                  <th className="text-left py-2 px-3 text-dark-400">Worker</th>
                  <th className="text-left py-2 px-3 text-dark-400">Loss</th>
                  {trainingConfig?.config?.taskType === 'classification' && (
                    <th className="text-left py-2 px-3 text-dark-400">Accuracy</th>
                  )}
                </tr>
              </thead>
              <tbody>
                {metrics.slice(-20).map((m, i) => (
                  <tr key={i} className="border-b border-dark-800">
                    ...
                    <td className="py-2 px-3 text-primary-400">{m.loss?.toFixed(6)}</td>
                    {trainingConfig?.config?.taskType === 'classification' && (
